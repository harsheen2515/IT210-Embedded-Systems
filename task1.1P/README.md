...
## Functions Used
### setup():
Sets the button as an input and the two LED pins as outputs.

### turnLightsOn():
Turns both LEDs on, records the starting time and indicates that the lighting sequence is running.

### checkLights():
Checks the elapsed time and turns the porch light off after 30 seconds and the hallway light off after 60 seconds.

### loop():
Checks whether the button has been pressed and starts the lighting sequence when appropriate. It also calls checkLights() while the sequence is running.

### turnLightsOn():
While the lighting sequence is running, loop() calls checkLights() so that the program can monitor the timing of both lights.
